import { getAllComplaints } from '../../api/admin/dashboard/complaints.js';
...
export async function exportComplaintsToExcel(preparedById, approvedById) {
    try {
        ...
        const worksheetData = [
            [barangayName],
            [cityProvince],
            ['Barangay Blotter & Complaints Directory'],
            [''],
            ['No.', 'Complainant', 'Respondent / Appellant', 'Incident Details', 'Status', 'Date Filed'],
        ];

        const HEADER_ROW = 4;

        const rows = complaints.map((complaint, index) => {
            const dateFiled = new Date(complaint.dateCreated);
        ...
        worksheetData.push(['Total Complaints:', '', complaints.length]);
        worksheetData.push([]);
        ...
        worksheet['!merges'] = [
            ...
            { s: { r: prepByRow, c: 3 }, e: { r: prepByRow, c: 5 } },
            { s: { r: lineRow, c: 0 }, e: { r: lineRow, c: 1 } },
            { s: { r: lineRow, c: 3 }, e: { r: lineRow, c: 5 } },
            { s: { r: labelRow, c: 0 }, e: { r: labelRow, c: 1 } },
            { s: { r: labelRow, c: 3 }, e: { r: labelRow, c: 5 } },
            { s: { r: positionRow, c: 0 }, e: { r: positionRow, c: 1 } },
            { s: { r: positionRow, c: 3 }, e: { r: positionRow, c: 5 } },
        ];
        ...
        const filename = `complaints_export_${timestamp}.xlsx`;
